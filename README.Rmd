...
> An attempt at organizing some `spaCy` workflows.  Some functions for disentangling `spaCy` output.  For working with actual corpora, as opposed to `nlp("This is a sentence.")`.  
    ...
    for ix, doc in enumerate(docs):
      for sent_i, sent in enumerate(doc.sents):
        ...
        for nc in sent.ents:
          details_dict["doc_id"].append(ix)
          details_dict["sent_id"].append(sent_i)
          details_dict["entity"].append(nc.text)
          details_dict["is_historical"].append(nc.is_historical)
          details_dict["start"].append(nc.start)
          details_dict["end"].append(nc.end)
